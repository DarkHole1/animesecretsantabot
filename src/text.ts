...
export const CREATE_CHAT = `Обычно Тайный Санта проводится в каком-то чате, поэтому удобно пересылать отзывы на тайтлы туда. Можете выбрать чат для пересылки или пропустить этот шаг с помощью команды /next. Не забудьте добавить меня туда, если всё таки решитесь.`

export const CREATE_RULES = `Теперь напишите правила. Этот пост будет использоваться как приветствие для присоединяющихся людей. Не забудьте написать там ограничения.`

export const CREATE_RESTRICTIONS = `Ограничения текстом - это хорошо, но автоматически проверять их ещё лучше! Напишите ограничение на тайтлы.`

export const CREATE_RESTRICTIONS_SUCCESS = `Успешно добавили ограничения!`

export const CREATE_RESTRICTIONS_FAILURE = `Не удалось распознать ограничения, попробуйте ещё раз...`

export const CREATE_OPTIONS = `Завершающие штрихи. Можете добавить пару опций для вашего мероприятия или просто перейти дальше через /next.`

export const CREATE_FINISH = (link: string) =>
